...
( function( window, undefined ) {
    ...
    window.brytescore.load = function ( url ) {
        ...
        //get just the events object or the package
        ...
        //window.brytescore.factory is a closure so that prop can be passed in and keep its value at the time of passing it in
        for(var prop in jsonEvents) {
            window.brytescore.factory = function( eventName, jsonEvent, packageGlobal, packageNamespace ){
                return function() {
                    var passTest = 0;
                    for( var jsonGlobalProp in packageGlobal) {
                        console.log('look here !!!!!!!!!!!!!!!!!!!!!');
                        console.log(packageGlobal);
                        console.log(jsonGlobalProp);
                        console.log(packageGlobal[jsonGlobalProp]);
                        for ( var jsonEventProp in jsonEvent ) {
                            for ( var args in arguments[0] ) {
                                var argPassed = false;
                                if ( 'globals' == jsonEventProp ) {
                                    if( jsonGlobalProp == jsonEvent[jsonEventProp]) {
                                        for ( var globalProp in packageGlobal[jsonGlobalProp]) {
                                            if ( 'required' == globalProp ) {
                                                for( var requiredGlobalProp in packageGlobal[jsonGlobalProp][globalProp]) {
                                                    if(args == requiredGlobalProp) {
                                                        if(packageGlobal[jsonGlobalProp][globalProp][requiredGlobalProp] == typeof arguments[0][args] ) {
                                                            passTest++;
                                                            argPassed = true;
                                                            alert('yep');
                                                        }
                                                    }
                                                    console.log( requiredGlobalProp );
                                                    console.log( packageGlobal[jsonGlobalProp][globalProp][requiredGlobalProp] );
                                                }
                                            }
                                            if ( 'optional' == globalProp ) {
                                                console.log( packageGlobal[jsonGlobalProp][globalProp] );
                                            }
                                ...
                                if ( 'required' == jsonEventProp ) {

                                }
                                if ( 'optional' == jsonEventProp ) {

                                }
                                if( !argPassed ) {
                                    console.log( packageNamespace + "." + eventName + " has an item named " + args + " that was not a defined property." )
                                }
                            }
                        }
                    }
            ...
            window.brytescore[namespace][prop] = window.brytescore.factory( prop, jsonEvents[prop], jsonGlobals, namespace );
