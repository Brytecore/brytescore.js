...
( function ( window, undefined ) {
	...
	//url = 'http://api.brytecore.net',	// Path to the API
		...
		heartbeatTimeout = false,           // boolean for whether the heartbeat timed out
...
//        readyFired = false,
//        docReady = false;                   // variable for when the document is ready. think Jquery $.ready()
	...
	window.brytescore = function () {
		...
		if ( functionName.indexOf( '.' ) > 0 ) {
			...
			if ( 'object' === typeof window.brytescore[namespace] ) {
				method = window['brytescore'][namespace][prop];
			} else {
				method = null;
			}
		} else {
...
//    };
	...
	function killSession() {
		brytescore.track( 'Heartbeat_stopped', {} );
	...
	function removeEventListeners() {
		brytescore.track( 'Heartbeat_restarted', {} );
		heartbeatID = window.setInterval( function () {
			brytescore.heartBeat();
		}, 15000 );
	...
	var startInactivityTimer = function () {
		if ( inactivityID === 0 ) {
			if ( eventListenerHandle !== undefined ) {
			...
			if ( heartbeatTimeout === true ) {
				window.brytescore.heartBeat();
			}
	...
	function addEventListeners() {
		...
		brytescore.track( 'Heartbeat_paused', {} );
